...
(function () {
  ...
  const LEGACY_KEY = "etad_state";
  const keyFor = (rev) => `etad_state_v${rev}`;
  let currentRev = 0;                      // the published revision this session is based on

  /* Fetch the repo-published snapshot (what every visitor sees). */
  async function fetchPublished() {
    try {
      const res = await fetch("state.json?ts=" + Date.now(), { cache: "no-store" });
      if (!res.ok) return null;
      const obj = await res.json();
      if (!obj || !Array.isArray(obj.sections) || typeof obj.weeks !== "object") return null;
      obj.schema = SCHEMA;
      return obj;
    } catch { return null; }
  }
  function save() {
    try { localStorage.setItem(keyFor(currentRev), JSON.stringify(state)); }
  ...
  function renderMastTools() {
    $("#mast-tools").innerHTML = `
      <button class="tbtn tbtn-primary" data-act="publish" title="Create the state.json to commit so everyone sees your updates">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M12 19V6m0 0l-6 6m6-6l6 6M5 3h14"/></svg>Publish</button>
      <button class="tbtn" data-act="export" title="Download a private backup of all your data">
      ...
      <button class="tbtn" data-act="reset" title="Discard local changes and reload the published version">
  ...
  function renderFoot() {
    const revLine = currentRev > 0
      ? `Published rev ${currentRev} \u00b7 your edits stay local until you Publish again`
      : `Not yet published \u00b7 use Publish to make this visible to everyone`;
    $("#foot").innerHTML = `
      ...
      <div style="margin-top:4px;color:var(--text-dim)">${revLine}</div>
  ...
  /* =========================================================
     ...
     ========================================================= */
  function onClick(e) {
    if (e.target.id === "publish-modal") { closeModal(); return; }
  ...
  /* Publish: hand the owner the exact state.json to commit to the repo. */
  function publish() {
    const newRev = currentRev + 1;
    const out = JSON.parse(JSON.stringify(state));
    out.rev = newRev; out.schema = SCHEMA; out.publishedAt = new Date().toISOString();
    downloadJSON(out, "state.json");
    showPublishModal(newRev);
  }

  function resetAll() {
    const hasPublished = currentRev > 0;
    const msg = hasPublished
      ? "Discard your local changes and reload the published version everyone else sees?"
      : "Reset everything back to the built-in seed data?";
    if (!confirm(msg)) return;
    if (!confirm("This cannot be undone. Continue?")) return;
    localStorage.removeItem(keyFor(currentRev));
    viewMonday = mondayOf(new Date());
    fetchPublished().then(pub => {
      state = pub || seed();
      state.schema = SCHEMA;
      if (!Number.isFinite(state.rev)) state.rev = currentRev;
      save(); renderAll();
      toast(hasPublished ? "Reloaded the published version" : "Reset to seed data");
    });
  }
  function exportState() {
    downloadJSON(state, `etad-backup-${weekKey(new Date())}.json`);
  ...
  /* ---------- publish modal ---------- */
  function showPublishModal(rev) {
    let m = $("#publish-modal");
    if (!m) {
      m = document.createElement("div");
      m.id = "publish-modal"; m.className = "modal-overlay";
      document.body.appendChild(m);
    }
    m.innerHTML = `
      <div class="modal">
        <button class="modal-x" data-act="modal-close" aria-label="Close">\u2715</button>
        <h3>Publish update \u2014 rev ${rev}</h3>
        <p><b>state.json</b> just downloaded. Put it in your repo so everyone visiting the link sees your latest content:</p>
        <ol>
          <li>Open your repo on GitHub (<span class="mono">ETAD-Space</span>).</li>
          <li>Click <b>Add file \u2192 Upload files</b>.</li>
          <li>Drag the downloaded <b>state.json</b> into the <b>root</b> of the repo (same folder as index.html), replacing the old one.</li>
          <li>Click <b>Commit changes</b>.</li>
          <li>Wait ~1 minute, then refresh the link. Everyone now sees <b>rev ${rev}</b>.</li>
        </ol>
        <p class="modal-note">Tip: keep editing anytime \u2014 your changes are saved in your browser. They only go live for others when you publish again.</p>
        <button class="modal-ok" data-act="modal-close">Got it</button>
      </div>`;
    m.classList.add("show");
  }
  function closeModal() { const m = $("#publish-modal"); if (m) m.classList.remove("show"); }
  ...
  async function init() {
    ...
    currentRev = (published && Number.isFinite(published.rev)) ? published.rev : 0;

    let working = null;
    const localRaw = localStorage.getItem(keyFor(currentRev));
    if (localRaw) { try { working = JSON.parse(localRaw); } catch {} }   // my draft on the current published rev
    if (!working && published) working = published;                       // published wins over any legacy copy
    if (!working) {                                                       // first-time migration of pre-publish edits
      const legacy = localStorage.getItem(LEGACY_KEY);
      if (legacy) { try { working = JSON.parse(legacy); } catch {} }
    }
    if (!working) working = seed();
